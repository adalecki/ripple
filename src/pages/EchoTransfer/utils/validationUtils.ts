...
function stringConversion(inputData: InputDataType) {
  ...
  for (const lineIdx in inputData.Barcodes) {
...
function patternsTabValidation(inputData: InputDataType, errors: string[]): Map<string, { replicates: number, concentrations: number[], isMatrix: boolean }> {
  ...
  for (const idx in inputData['Patterns']) {
...
function layoutTabValidation(inputData: InputDataType, testPlate: Plate, availablePatternNames: Map<string, { replicates: number, concentrations: number[], isMatrix: boolean }>, errors: string[]) {
  for (const idx in inputData['Layout']) {
    try {
...
function compoundsTabValidation(inputData: InputDataType, testPlate: Plate, availablePatternNames: Map<string, { replicates: number, concentrations: number[], isMatrix: boolean }>, errors: string[]): string[] {
  ...
  for (const idx in inputData['Compounds']) {
    ...
    try {
      if (isNaN(cpd['Volume (µL)'])) { errors.push(`${cpd['Compound ID']} on line ${parseInt(idx) + 2} of Compounds tab has a non-number Volume`); } else if (!(cpd['Volume (µL)'] > 0)) { errors.push(`${cpd['Compound ID']} on line ${parseInt(idx) + 2} of Compounds tab has a negative Volume`); }
    } catch (err) {
...
function barcodesTabValidation(inputData: InputDataType, srcBarcodes: string[], errors: string[]) {
  ...
  for (const idx in inputData['Barcodes']) {
    ...
    if (row['Destination Plate Barcodes']) {
      ...
      if (!barcodes.dest.includes(row['Destination Plate Barcodes'])) { barcodes.dest.push(row['Destination Plate Barcodes']); } else { errors.push(`Line ${parseInt(idx) + 2} of Barcodes tab has a repeated Destination barcode`); }
    }
    if (row['Intermediate Plate Barcodes']) {
      ...
      if (!barcodes.int.includes(row['Intermediate Plate Barcodes'])) { barcodes.int.push(row['Intermediate Plate Barcodes']); } else { errors.push(`Line ${parseInt(idx) + 2} of Barcodes tab has a repeated Intermediate barcode`); }
    }
  }
}
