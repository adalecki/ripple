...
function stringConversion(inputData: InputDataType) {
  for (const lineIdx in inputData.Recipes) {
    inputData.Recipes[lineIdx]['Name'] = (inputData.Recipes[lineIdx]['Name']?.toString().trim() || '');
  }
  for (const lineIdx in inputData.SourceLayout) {
  ...
  for (const lineIdx in inputData.Cocktails) {
...
function recipesTabValidation(inputData: InputDataType, testPlate: Plate, errors: string[], dropletSize?: number): string[] {
  ...
  for (const idx in inputData['Recipes']) {
...
function sourceLayoutTabValidation(inputData: InputDataType, testPlate: Plate, errors: string[]): string[] {
  ...
  for (const idx in inputData['SourceLayout']) {
    ...
    try {
      if (isNaN(row['Volume (µL)'])) { errors.push(`${row['Content']} on line ${parseInt(idx) + 2} of SourceLayout tab has a non-number Volume`); } else if (!(row['Volume (µL)'] > 0)) { errors.push(`${row['Content']} on line ${parseInt(idx) + 2} of SourceLayout tab has a negative Volume`); }
    } catch (err) {
...
function cocktailsTabValidation(inputData: InputDataType, availableRecipeNames: string[], contents: string[], errors: string[]) {
  for (const idx in inputData['Cocktails']) {
