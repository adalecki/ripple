...
describe('CocktailBuilder designer smoke test', () => {
  ...
  test('every tab renders without crashing', () => {
    renderCocktail();
    for (const name of ['Inventory', 'Cocktails', 'Build', 'Instructions', 'Recipes']) {
  ...
  test('the substitution generator is present and gated', () => {
    renderCocktail();
    fireEvent.click(screen.getByRole('tab', { name: 'Cocktails' }));
    const pane = activePane();
    ...
    expect(screen.getByText('Pick a base cocktail')).toBeInTheDocument();
  });

  test('a cocktail row can be added', () => {
    renderCocktail();
    fireEvent.click(screen.getByRole('tab', { name: 'Cocktails' }));

    fireEvent.click(activePane().getByRole('button', { name: 'Add Cocktail' }));
    const table = activePane().getByRole('table');
    // header row plus the one cocktail we just added
